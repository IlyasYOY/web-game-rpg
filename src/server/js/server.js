    ...
    let moveHandler = {
        ...
        botTurn(id) {
            ...
            let nearObject = function (arr) {
                let nearObj = 0;
                let minLength = findPath(mapHandler.currentMap, [io.bots[id].player.x, io.bots[id].player.x], [arr[0][0], arr[0][1]]);
                for (let i in arr) {
                    let newDistance = findPath(mapHandler.currentMap, [io.bots[id].player.x, io.bots[id].player.x], [arr[i][0], arr[i][1]]);
                    if (newDistance < minLength) {
                        minLength = newDistance;
                        nearObj = i;
                    }
                }
            }
            console.log(getObjects()[nearObject(getObjects())]);
        },
        createNewPlayer(id) {
